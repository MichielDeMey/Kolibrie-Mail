...
{
    internal sealed class TextReader : ISectionReader
    {
        #region ISectionReader Members

        public string ReadSection(ImapResponseReader reader)
        {
            var sizeString = Regex.Match(reader.CurrentLine, RegexPatterns.CurlyParanthesizedSizePattern).Value.Trim(new[] {Characters.CurlyClosedBracket, Characters.CurlyOpenBracket});
        ...
        {
            return line.Contains("BODY[TEXT]") || line.Contains("BODY.PEEK[TEXT]");
        }
